...
import type { CrawlerResult, TypeMapping, Retailer, ProductType, Province } from './types'
...
export class VangTodayCrawler extends BaseCrawler {
  ...
  async fetchPrices(): Promise<CrawlerResult> {
    ...
    try {
      ...
      await crawlerLogger.updateLog(logId, {
        ...
        errorMessage: errors.length > 0 ? `${errors.length} items failed to parse` : undefined,
      })
   ...
   */
  private async parsePrices(
    apiResponse: VangTodayResponse
  ): Promise<{ prices: PriceData[]; errors: Array<{ item: string; error: string }> }> {
    const prices: PriceData[] = []
    const errors: Array<{ item: string; error: string }> = []

    // Fetch all enabled mappings, retailers, provinces, product types
    const { mappings, retailers, provinces, productTypes } = await this.fetchReferenceData()
      ...
      try {
        ...
        if (typeCode === 'XAUUSD') {
          prices.push({
            id: '',
            createdAt: timestamp,
            retailer: mapping.retailerCode,
            province: provinceCode,
            productType: mapping.productTypeCode,
  ...
  private async fetchReferenceData(): Promise<{
    ...
    const [mappingsResult, retailersResult, provincesResult, productTypesResult] =
      await Promise.all([
        supabase

  ...
  return useMutation({
    mutationFn: async (entry: {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.details || error.error || 'Failed to create portfolio entry')
      }
...
export function useUpdatePortfolioEntry() {
  ...
  return useMutation({
    mutationFn: async (entry: {
      ...
      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.details || error.error || 'Failed to update portfolio entry')
      }
...
export function useDeletePortfolioEntry() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.details || error.error || 'Failed to delete portfolio entry')
      }

...
import type { PortfolioGrowthResponse, PortfolioGrowthDataPoint, PortfolioEntry } from '@/lib/types'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupBy = (searchParams.get('groupBy') || 'total') as 'total' | 'monthly' | 'yearly'
    ...
    const uniqueGoldTypes = new Map<string, { retailer: string; productType: string; province: string | null }>()
    portfolioEntries.forEach((entry) => {
    ...
    for (const date of dates) {
      for (const { retailer, productType, province } of uniqueGoldTypes.values()) {
    ...
    if (groupBy === 'total') {
      ...
      groupedData = Array.from(monthlyMap.values()).sort((a, b) => a.date.localeCompare(b.date))
      ...
      growthData.forEach((point) => {
        ...
        if (!yearlyMap.has(yearKey) || new Date(point.date) > new Date(yearlyMap.get(yearKey)!.date)) {
      ...
      groupedData = Array.from(yearlyMap.values()).sort((a, b) => a.date.localeCompare(b.date))
    }

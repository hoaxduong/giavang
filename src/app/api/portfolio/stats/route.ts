...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalGoldAmount = portfolioEntries.reduce((sum, entry) => sum + Number(entry.amount), 0)
    ...
    const profitLossPercent = totalVndInvested > 0 ? (profitLossVnd / totalVndInvested) * 100 : 0

    const soldEntries = portfolioEntries.filter((entry) => entry.sold_at).length
    const activeEntries = portfolioEntries.filter((entry) => !entry.sold_at).length

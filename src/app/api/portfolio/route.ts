...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!priceAtTime) {
      return NextResponse.json(
        {
          error: 'Could not find price data for the specified time and gold type',
        },
        { status: 400 }
      )
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const { id, sold_at, amount, retailer, product_type, province, bought_at } = body
    ...
    if (bought_at !== undefined) {
      ...
      const newPrice = await getPriceAtTimestamp(
        ...
        updateData.province !== undefined ? updateData.province : existing.province,
        bought_at
      )
      if (newPrice) {

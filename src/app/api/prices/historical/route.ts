...
export async function GET(request: NextRequest) {
  try {
    ...
    const aggregatedData = aggregateByInterval(data || [], interval as 'hourly' | 'daily' | 'weekly')
...
function aggregateByInterval(
  ...
  for (const [key, groupPrices] of grouped) {
    const avgBuyPrice = groupPrices.reduce((sum, p) => sum + Number(p.buy_price), 0) / groupPrices.length
    const avgSellPrice = groupPrices.reduce((sum, p) => sum + Number(p.sell_price), 0) / groupPrices.length
